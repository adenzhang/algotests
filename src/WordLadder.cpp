...
typedef deque<StringVectorPtr> StringVectorQ;
typedef vector<vector<string>> StringVectorVector;
...
struct LinkedNode {
    ...
    vector<LinkedNodePtr> children;
    DataType data;

    LinkedNode(const DataType& d)
        : data(d)
    {
    }
...
typedef LinkedNode<std::string> StringNode;
typedef typename StringNode::LinkedNodePtr StringNodePtr;
typedef vector<StringNodePtr> StringNodeVector;
...
public:
    ...
    bool exists(string& s, StringVectorQ& q1, StringVectorQ& q2)
    {
        for (StringVectorQ::iterator it = q1.begin(); it != q1.end(); ++it) {
            if (std::find((*it)->begin(), (*it)->end(), s) != (*it)->end())
                return true;
        }
        for (StringVectorQ::iterator it = q2.begin(); it != q2.end(); ++it) {
            if (std::find((*it)->begin(), (*it)->end(), s) != (*it)->end())
                return true;
        }
        return false;
    ...
    {
        typedef unordered_map<string, StringNodePtr> StringNodeMap;
        const int L = end.size();
        StringNodePtr root(new StringNode(start));
        StringNodePtr endNode((new StringNode(end))); //root->createChild(end);
        StringNodeMap allnodes = { { start, root }, { end, endNode } };
        StringNodeMap currNodes = { { start, root } };
        int depth = 0;
        while (shortest < 0 && !currNodes.empty()) {
            ++depth;
            StringNodeMap newNodes;
            for (StringNodeMap::iterator it = currNodes.begin(); it != currNodes.end(); ++it) {
                StringNodePtr node = it->second;
                string s = node->data;
                for (int i = 0; i < L; ++i) {
                    string newStr = s;
                    char ch = s[i];
                    do {
                        ...
                        if (dict.end() != dict.find(newStr) && allnodes.find(newStr) == allnodes.end()) { // continue with next search
                            if (newNodes.find(newStr) == newNodes.end()) {
                                StringNodePtr p = node->createChild(newStr);
                                newNodes.insert(std::make_pair(newStr, p));
                            } else {
                                node->addChild(newNodes[newStr]);
                            }
                        }
                    } while (true);
                }
            }
            allnodes.insert(newNodes.begin(), newNodes.end());
            currNodes = std::move(newNodes);
        }
        StringVectorVector result;
        struct GenerateVector {
            static void gen(StringNodePtr end, StringNodePtr root, StringVectorVector& vv, StringVector v)
            {
                ...
                for (int i = 0; i < root->children.size(); ++i) {
                    gen(end, root->children[i], vv, v);
                }
            }
        };

        if (shortest > 0) {
            GenerateVector::gen(endNode, root, result, StringVector());
        }
        // todo: clear tree root
        return result;
    }
...
TEST_FUNC(wordLadder_test, "wordLadder_test")
{
    //    string start[2] = {"hit", "cog"};
    //    StringSet dict = {"hot","dot","dog","lot","log"};

    //    string start[2] = {"a", "c"};
    //    StringSet dict = {"a","b","c"};

    string start[2] = { "sand", "acne" };
    StringSet dict = {
    ...
    REQUIRE(false);
    Solution sln;
    StringVectorVector result = sln.findLadders(start[0], start[1], dict);
