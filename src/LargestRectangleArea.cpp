...
public:
    ...
    long arrayManipulation( int n, vector<vector<int>> queries )
    {
        ...
        const auto &getPos = [&]( const endpoint &x ) { return x.bStart == 1 ? queries[x.idx][0] : ( queries[x.idx][1] + 1 ); };
        const auto &getVal = [&]( const endpoint &x ) { return queries[x.idx][2] * x.bStart; };
        const auto &cmp = [&]( const endpoint &x, const endpoint &y ) { return getPos( x ) < getPos( y ); };
    ...
    {
        const auto &nextDiff = []( const char *p ) {
            auto n = p;
            while ( *n && *p == *n )
                ++n;
            return n;
...
TEST_FUNC( arrayManipulation_tests )
{
    Solution sln;
    {
        IntVecVec A{{1, 5, 3}, {4, 8, 7}, {6, 9, 1}};
        REQUIRE( sln.arrayManipulation( 0, A ) == 10 );
    }
    {
        IntVecVec A{{1, 2, 100}, {2, 5, 100}, {3, 4, 100}};
        REQUIRE( sln.arrayManipulation( 0, A ) == 200 );
    }
    {
        IntVecVec A{{2, 3, 603}, {1, 1, 286}, {4, 4, 882}};
