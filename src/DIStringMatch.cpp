...
public:
    ...
    int longestNoneDescreasing(ArrT& A, const function<int64_t(const E&)>& getH = [](const E& x) { if constexpr( is_convertible_v<E, int64_t> ) return x; else return 0; })
    {
    ...
    template <typename ArrT, typename E = typename ArrT::value_type>
    int longestIncreasing(ArrT& A, const function<int64_t(const E&)>& getH = [](const E& x) { if constexpr( is_convertible_v<E, int64_t> ) return x; else return 0; })
    {
        using HightT = int64_t;
        auto N = A.size();
        if (N < 2)
